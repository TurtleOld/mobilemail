...
class JmapOAuthClient(
            ...
            if (response.code == 401 || response.code == 403) {
                try {
                    ...
                    if (!retryResponse.isSuccessful) {
                        ...
                        throw Exception("JMAP request failed после обновления токена: код ${retryResponse.code}, ответ: ${LogRedactor.redact(retryBody.take(200))}")
                    }
    ...
    override suspend fun sendEmail(
        ...
        // После успешного EmailSubmission/set переносим письмо из Drafts в Sent.
        // Некоторые серверы не делают это автоматически, поэтому делаем явно.
        try {
            val mailboxes = getMailboxes(targetAccountId)
            ...
            // Не падаем: отправка уже успешна, но логируем, чтобы можно было диагностировать
    ...
    override suspend fun getEmailSubmission(
        ...
        // Попытка вытащить человекочитаемую причину из deliveryStatus.*.smtpReply/smtpReply
        ...
        // delivered/failed в JMAP нет стандарта; некоторые серверы отдают undoStatus/isFinal/status
